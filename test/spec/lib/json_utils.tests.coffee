...
describe 'JSONUtils @quick @json', ->
  it 'parse maintains types correctly', (done) ->
    parsed_json = JSONUtils.fromQuery(JSONUtils.toQuery(VALUES))
  ...
  it 'toQuery handles control directives', (done) ->
    ...
    assert.deepEqual(JSONUtils.fromQuery(JSONUtils.toQuery(query)), parsed_query, 'matches without extra JSON operations')
    assert.deepEqual(JSONUtils.fromQuery(JSON.parse(JSON.stringify(JSONUtils.toQuery(query)))), parsed_query, 'matches with extra JSON operations')
    done()

  describe 'Strict JSON in URL', ->
    it 'handles unicode strings', (done) ->
      KEY = 'string'
      url_parts = URL.parse(MODEL_URL, true); _.extend(url_parts.query, JSONUtils.toQuery(_.pick(VALUES, KEY))); url = URL.format(url_parts)
      assert.equal(url, "#{MODEL_URL}?#{KEY}=#{encodeURIComponent('"' + VALUES[KEY] + '"')}", 'URL has string')
      url_parts = URL.parse(url, true)
      assert.equal(JSONUtils.fromQuery(url_parts.query)[KEY], VALUES[KEY], 'Same type returned')
      done()
